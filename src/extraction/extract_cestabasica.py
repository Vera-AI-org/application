...
def extrair_texto_pdf(caminho_pdf):
    ...
    return paginas
"""# 2 Separando assinaturas e recibos"""
...
"""#3 Verificando quem recebeu e não assinou"""
...
def normalize_name(name):
    # Normaliza Unicode (remove acentos) e converte para minúsculas
...
def main(input_path: str, todos_funcionarios: list):    
    paginas = extrair_texto_pdf(input_path)
    recibos, assinaturas = separate_pdf_content_from_list(paginas)
    return check_receipts_and_signatures(recibos, assinaturas, todos_funcionarios, "cestas_basicas.csv")
...
if __name__ == "__main__":
    ...
    main(args.input_path, args.all_employers)
